...
# 23.5 Patch Telegram/BUILD to silence validator warnings 90683 (missing
# NSLocationAlwaysAndWhenInUseUsageDescription, which blocks TestFlight
# install) and 90626 (missing Siri example phrases). Both info.plists for
# the main app and the SiriIntents extension are generated by Bazel from
# inline plist_fragment templates in Telegram/BUILD, so patching the
# static Telegram/Telegram-iOS/Info.plist has no effect on the compiled IPA.
echo "  [23.5/24] Patching Telegram/BUILD plist_fragments (90683 + 90626)..."
TG_BUILD="${TARGET_DIR}/Telegram/BUILD"
if [ -f "$TG_BUILD" ]; then
    python3 "${SCRIPT_DIR}/patch_telegram_build.py" "$TG_BUILD"
else
    echo "    FATAL: $TG_BUILD not found"
    exit 1
fi
